...
from contracts.tracker.async_token_tracker_engine import (
...
@sp.add_test(name="FA2 Engine Settlement Premium")
def testFA2EngineSettlementPremium():
    ...
    tracker_engine = AsyncTokenTrackerEngine(
        synth.address,
        token_id,
        synth.address,
        collateral_token_id,
    ...
    options_listing = TokenOptionsListing(
        synth.address,
        token_id,
        synth.address,
        collateral_token_id,
    ...
    scenario += tracker_engine.set_contracts(
        ...
        observed_price_oracle=observed_oracle.address,
    ...
    price = 1000000
    gov_tokens_alice = (
        Constants.GOVERNANCE_TOKEN_ISSUANCE_RATE * Constants.SECONDS_PER_WEEK
    )
    ...
    scenario.verify_equal(
        ...
        < 10**7,
        True,
    )  # fixed point arithmentic we allow an error...
    ...
    price = 800000
    scenario += target_oracle.set_price(
        price
    )  # the price has dropped by 20% we can re-touch the vaults.
    scenario += tracker_engine.touch([alice.address, bob.address])
    scenario.verify_equal(
        governance_token.data.stakes[alice.address],
        33 * price * tracker_engine.data.compound_interest_rate,
    )
    scenario.verify_equal(governance_token.data.stakes.contains(bob.address), False)
    ...
    scenario += target_oracle.set_price(
        price
    )  # the price back to normal we can re-touch the vaults.
    scenario += tracker_engine.touch([alice.address, bob.address])
    scenario.verify_equal(
        governance_token.data.stakes[alice.address],
        33 * price * tracker_engine.data.compound_interest_rate,
    ...
    scenario += options_listing.execute_intent(
        address=bob.address, token_amount=11 * Constants.PRECISION_FACTOR
    ).run(sender=bob, now=now, valid=False)

    scenario.p("Bob settles on alice (and succeeds")
    scenario += options_listing.execute_intent(
        address=alice.address, token_amount=11 * Constants.PRECISION_FACTOR
    ).run(sender=bob, now=now)
