...
def test():
    ...
    scenario += token.mint(
        ...
        token_amount=1000 * Constants.PRECISION_FACTOR,
    )
    scenario += token.mint(
        owner=dan.address,
        token_id=token_id,
    ...
    fee_amount = (5 * Constants.PRECISION_FACTOR) >> Constants.BID_FEE_BITSHIFT
    matching_amount = sp.as_nat(5 * Constants.PRECISION_FACTOR - fee_amount)
    scenario.p("Cannot fullfill too late")
    scenario += options_listing.fulfill_intent(
        address=alice.address, collateral_token_amount=matching_amount
    ).run(sender=bob, now=sp.timestamp(2 * 24 * 60 * 60 + 1), valid=False)
    scenario.p("Can if amount is right")
    scenario += options_listing.fulfill_intent(
        address=alice.address, collateral_token_amount=matching_amount
    ).run(sender=bob, now=sp.timestamp(0), valid=True)
    ...
    fee_amount = (2 * Constants.PRECISION_FACTOR) >> Constants.BID_FEE_BITSHIFT
    matching_amount = sp.as_nat(2 * Constants.PRECISION_FACTOR - fee_amount)
    scenario += options_listing.fulfill_intent(
        address=alice.address, collateral_token_amount=matching_amount
