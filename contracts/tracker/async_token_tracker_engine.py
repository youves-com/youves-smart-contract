...
from utils.fa2 import LedgerKey, RecipientTokenAmount, AdministrableMixin
...
class AsyncTokenTrackerEngine(sp.Contract, AdministrableMixin, InternalMixin):
    ...
    def internal_liquidate(self, liquidation):
        ...
        liquidate_threshold.value = (
            liquidate_threshold.value // self.price_extra_precision_factor
        )
        ...
        token_amount_market_value = (
            liquidation.token_amount * self.data.target_price
        ) // self.price_extra_precision_factor
        liquidation_reward = (
    ...
    def withdraw(self, token_amount):
        ...
        sp.transfer(token_amount, sp.mutez(0), sp.self_entry_point("internal_withdraw"))

    @sp.entry_point(check_no_incoming_transfer=True)
    def internal_withdraw(self, token_amount):
    ...
    def internal_settle_with_vault(self, settlement):
        ...
        market_price_amount = sp.local(
            "market_price_amount",
            (settlement.token_amount * self.data.target_price)
            // self.price_extra_precision_factor,
        )
        ...
        sp.verify(
            payout_amount.value >= Constants.MIN_COLLATERAL_AMOUNT_THRESHOLD,
            message=Errors.AMOUNT_TOO_SMALL,
        )
    ...
    def touch(self, addresses):
        ...
        self.fetch_target_price(sp.unit)
        self.update_accrual(sp.unit)
        sp.transfer(addresses, sp.mutez(0), sp.self_entry_point("internal_touch"))

    @sp.entry_point(check_no_incoming_transfer=True)
    def internal_touch(self, addresses):
        """triggers the contract to update the accrual
        Post: update_accrual()
        """
        sp.set_type(addresses, sp.TList(sp.TAddress))
        ...
        with sp.for_("address", addresses) as address:
            with sp.if_(self.data.vault_contexts[address].allows_settlement):
                self.update_governance_stake(
                    sp.record(
                        address=address,
                        amount=self.data.vault_contexts[address].minted
                        * self.data.compound_interest_rate
                        * self.data.target_price
                        // (
                            Constants.PRECISION_FACTOR
