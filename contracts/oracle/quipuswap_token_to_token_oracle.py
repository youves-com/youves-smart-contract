...
class QuipuswapTokenToTokenOracle(sp.Contract, InternalMixin):
    """
    ...
    def get_init_storage(self):
        ...
        storage["pair_id"] = self.pair_id
        storage["price"] = 0
        return storage

    def __init__(self, dex_contract, pair_id, token1_decimals, token2_decimals):
    ...
    def set_reserves(self, reserves):
        ...
        sp.verify(sp.sender == self.data.dex_contract, message=Errors.NOT_ADMIN)
        self.data.price = (
            sp.fst(reserves) * sp.nat(10**self.token2_decimals) * Constants.PRICE_PRECISION_FACTOR
    ...
    def get_price(self, callback):
        ...
        get_reserve_balance = sp.contract(
            sp.TPair(sp.TNat, sp.TContract(sp.TPair(sp.TNat, sp.TNat))),
            self.data.dex_contract,
            entry_point="get_reserves",
        ).open_some()
        ...
        sp.transfer(
            (self.data.pair_id, set_reserves_callback_contract),
            sp.mutez(0),
            get_reserve_balance,
        )
